...
* **Company Size Normalization**: Replaced abbreviated code values in `company_size` (`S`, `M`, `L`) with descriptive text labels (`Small`, `Medium`, `Large`).
* **Boolean Flag Standardization**: Converted text/boolean conditional values (e.g., `manages_people`, `uses_ai_tools_daily`, `switched_jobs_last_year`) into standard binary numeric flags (`1` and `0`) for reliable measure aggregation.
* **Surrogate Key Generation**: Created unique index keys across all dimension tables (e.g., `comp_loc_id`, `emp_type_id`, `job_id`, `exp_lvl_id`) to establish clean one-to-many dimensional relationships.
